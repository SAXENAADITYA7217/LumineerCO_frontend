...
const InProgressRequests = () => {
    ...
    const fetchAcceptedRequests = async () => {
        try {
            const response = await axios.get(
                `${backendURL}/admin/get-request-by-status?status=IN_PROGRESS`
            );
    ...
    return (
        <div className="container-fluid py-4">
            {/* Header section with Blue Info Theme */}
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h2 className="fw-bold text-dark m-0">In-Progress Requests</h2>
                    <p className="text-muted m-0 small">Repairs that are currently being worked on</p>
                </div>
                <span className="badge bg-info text-dark px-3 py-2 rounded-pill fw-bold fs-6">
            ...
            {requests.length === 0 ? (
                <div className="alert alert-info border-0 shadow-sm p-4 text-center">
                    <strong>No In-Progress Requests Found</strong>
                    <p className="text-muted mb-0 small mt-1">Tickets currently under repair will show up here.</p>
                </div>
            ) : (
                <div className="row g-4">
                    {requests.map((request) => (
                        <div key={request.repairId} className="col-12 col-md-6 col-xl-4">
                            {/* Card with Blue Top Border to match "In Progress" theme */}
                                    ...
                                    <div>
                                        ...
                                        <div className="d-flex justify-content-between align-items-start mb-3">
                                            <div>
                                                ...
                                                <span className="badge bg-info text-dark fw-bold px-2 py-1 small">
                                                    {request.status}
                                                </span>
                                            </div>
                                            <button
...
export default InProgressRequests;